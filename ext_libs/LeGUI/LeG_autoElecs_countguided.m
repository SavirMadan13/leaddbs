...
% Stable peak/chain sEEG detector.
% Runs count-free by default. Bright compact CT peaks seed shaft hypotheses;
% the surface mesh gates candidates and contributes to shaft/contact scoring,
% but it is not used as a hard tube-first front end. If explicitly requested
% in 'countguided' mode and expected per-electrode counts are available,
% those counts are used as priors for shaft/contact fitting.
...
fprintf('\n[AutoElec-Chain] === Peak/chain detection ===\n');
if useCountGuidance
    fprintf('[AutoElec-Chain] Using count priors: %d depth electrodes  %d contacts\n', ...
        numel(expected.counts), sum(expected.counts));
elseif useChainOptimization
    fprintf('[AutoElec-Chain] Running chain-optimization selection (experimental)\n');
else
    fprintf('[AutoElec-Chain] Running count-free (no electrode-count priors)\n');
end
fprintf('[AutoElec-Chain] Policy: surface mesh gates/scoring only; no hard tube-first front end\n');
...
fprintf('[AutoElec-Chain] Candidates: %d (inside=%d, nearSurface=%d)\n', ...
    size(candMM, 1), sum(candInside), sum(candNear));
fprintf('[AutoElec-Chain] Surface support fraction: %.2f\n', surfaceSupport);

if isempty(candMM)
    WC = zeros(0, 3);
    T = (TMax + TMin) / 2;
    fprintf('[AutoElec-Chain] No candidates survived peak extraction.\n');
    return;
end

chains = proposeChainsCountGuided(candMM, candConf, candInside, candNear);
fprintf('[AutoElec-Chain] Shaft hypotheses: %d\n', numel(chains));
...
if isempty(shafts)
    WC = zeros(0, 3);
    T = (TMax + TMin) / 2;
    fprintf('[AutoElec-Chain] No shafts survived fitting.\n');
    return;
end
...
if size(finalMM, 1) ~= size(WC, 1)
    fprintf('[AutoElec-Chain] Bounds filter: kept %d / %d contacts within draw-safe MR bounds\n', ...
        size(WC, 1), size(finalMM, 1));
end

fprintf('[AutoElec-Chain] FINAL: %d shafts, %d contacts in %.1f s\n', ...
...
if nContactsAfterTrim ~= nContactsBeforeTrim
    fprintf('[AutoElec-Chain] Outside trim: %d -> %d contacts after removing extracranial shaft ends\n', ...
...
if ~isempty(rescue)
    ...
    fprintf('[AutoElec-Chain] Intracranial rescue: added %d shafts from unused fits\n', ...
        numel(rescue));
end
end
...
if dropWholeOutside > 0
    fprintf('[AutoElec-Chain] Outside reject: dropped %d shafts with no intracranial core\n', ...
        dropWholeOutside);
end
if trimmedAny > 0
    fprintf('[AutoElec-Chain] Outside end trim touched %d shafts\n', trimmedAny);
end
end

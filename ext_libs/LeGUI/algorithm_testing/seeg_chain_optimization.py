...
import numpy as np
import seeg_contact_detector as baseline_detector
...
def propose_component_chains(
    ...
    for label_id in cand_labels:
        ...
        cand_idx = cand_idx[cands.inside_mask[cand_idx] | cands.near_surface_mask[cand_idx]]
        if cand_idx.size == 0:
            continue
        ...
        regularity = float(max(reg_obs, 0.45 * reg_path, min(0.32, 0.14 + 0.008 * span_mm)))
        brain_support = float(
            max(
...
def select_and_fit_shafts(
    ...
    candidate_chains = chains[: min(len(chains), 180)]
    ...
    candidate_pool: list[FittedShaft] = []
    for shaft in fits:
        if shaft.score < score_thr - 0.45 and shaft.evidence_score < 0.82:
            continue
        ...
        if shaft.brain_support < min_brain_support and shaft.evidence_score < brain_evidence_bypass:
            continue
        if shaft.outside_fraction > max_outside_fraction + 0.12 and shaft.evidence_score < 0.95:
            continue
        candidate_pool.append(shaft)

    selected = optimize_shaft_subset(
        candidate_pool,
        max_total_contacts=max_total_contacts,
        surface_support=surface_support,
        use_seed_conflicts=True,
        allow_fragment_conflicts=False,
        beam_width=256,
    )
    ...
    selected = inflate_selected_shafts(selected, ct_raw, ct_blob, ct_affine, surface, surface_support)
    ...
    total_contacts = int(sum(shaft.inferred_contacts for shaft in selected))
    mean_contacts = (total_contacts / len(selected)) if selected else 0.0
    if len(selected) >= 15 and mean_contacts >= 10.5:
        selected = optimize_shaft_subset(
            selected,
            max_total_contacts=max_total_contacts,
            surface_support=surface_support,
            use_seed_conflicts=False,
            allow_fragment_conflicts=True,
            beam_width=128,
        )
...
def optimize_shaft_subset(
    shafts: list[FittedShaft],
    max_total_contacts: int,
    surface_support: float,
    use_seed_conflicts: bool,
    allow_fragment_conflicts: bool,
    beam_width: int,
) -> list[FittedShaft]:
    if not shafts:
        return []
    ...
    utilities = np.asarray(
        [shaft_selection_utility(shaft, surface_support) for shaft in shafts],
        dtype=np.float64,
    )
    order = np.argsort(-utilities)
    ordered_shafts = [shafts[int(i)] for i in order]
    ordered_utils = utilities[order]
    conflict_masks = build_shaft_conflict_masks(
        ordered_shafts,
        use_seed_conflicts=use_seed_conflicts,
        allow_fragment_conflicts=allow_fragment_conflicts,
    )

    n = len(ordered_shafts)
    suffix_pos = np.zeros(n + 1, dtype=np.float64)
    for i in range(n - 1, -1, -1):
        suffix_pos[i] = suffix_pos[i + 1] + max(0.0, float(ordered_utils[i]))

    states: list[tuple[float, int, int, int]] = [(0.0, 0, 0, 0)]
    for i in range(n):
        bit = 1 << i
        next_states: dict[tuple[int, int], tuple[float, int, int, int]] = {}
        for value, selected_mask, forbidden_mask, contact_total in states:
            skip_state = (value, selected_mask, forbidden_mask, contact_total)
            skip_key = (forbidden_mask, contact_total)
            if skip_key not in next_states or value > next_states[skip_key][0]:
                next_states[skip_key] = skip_state

            shaft = ordered_shafts[i]
            if forbidden_mask & bit:
                continue
            new_total = contact_total + shaft.inferred_contacts
            if new_total > max_total_contacts:
                continue
            new_value = value + float(ordered_utils[i])
            new_selected = selected_mask | bit
            new_forbidden = forbidden_mask | bit | conflict_masks[i]
            take_state = (new_value, new_selected, new_forbidden, new_total)
            take_key = (new_forbidden, new_total)
            if take_key not in next_states or new_value > next_states[take_key][0]:
                next_states[take_key] = take_state

        ranked = sorted(
            next_states.values(),
            key=lambda state: (state[0] + suffix_pos[i + 1], state[0]),
            reverse=True,
        )
        states = ranked[:beam_width]

    best_value, best_mask, _, _ = max(states, key=lambda state: state[0])
    del best_value
    selected = [ordered_shafts[i] for i in range(n) if best_mask & (1 << i)]
    selected.sort(key=lambda shaft: shaft.score, reverse=True)
    return selected


def shaft_set_quality(shafts: list[FittedShaft], surface_support: float) -> float:
    if not shafts:
        return -1e9

    surf_weight = 0.45 if surface_support >= 0.45 else 0.25
    total = 0.0
    short_count = 0
    total_contacts = 0
    for shaft in shafts:
        inside_fraction = float(
            getattr(
                shaft,
                "inside_fraction",
                max(0.0, float(shaft.brain_support) - 0.5 * float(shaft.outside_fraction)),
            )
        )
        total += (
            shaft.score
            + 0.30 * shaft.support_score
            + 0.35 * shaft.evidence_score
            + surf_weight * (0.80 * shaft.brain_support + 0.50 * inside_fraction - 1.00 * shaft.outside_fraction)
        )
        total_contacts += shaft.inferred_contacts
        if shaft.inferred_contacts <= 3:
            short_count += 1

    mean_contacts = total_contacts / max(len(shafts), 1)
    fragmentation_penalty = max(0.0, len(shafts) - total_contacts / 10.0)
    short_penalty = 0.45 * short_count
    spread_penalty = 0.0
    if mean_contacts > 12.5:
        spread_penalty = 0.25 * (mean_contacts - 12.5) * len(shafts)
    return float(total - 0.60 * fragmentation_penalty - short_penalty - spread_penalty)


def shaft_selection_utility(shaft: FittedShaft, surface_support: float) -> float:
    surf_weight = 0.45 if surface_support >= 0.45 else 0.25
    utility = (
        shaft.score
        + 0.35 * shaft.support_score
        + 0.55 * shaft.evidence_score
        + 0.10 * min(shaft.inferred_contacts, 12)
        + surf_weight * (0.90 * shaft.brain_support + 0.60 * shaft.inside_fraction - 1.10 * shaft.outside_fraction)
    )
    return float(utility)


def build_shaft_conflict_masks(
    shafts: list[FittedShaft],
    use_seed_conflicts: bool,
    allow_fragment_conflicts: bool,
) -> list[int]:
    masks = [0 for _ in shafts]
...
def shafts_conflict(
    ...
    if use_seed_conflicts:
        a_seed = set(a.seed_indices)
        b_seed = set(b.seed_indices)
        if a_seed and b_seed:
            overlap = len(a_seed & b_seed) / max(1, min(len(a_seed), len(b_seed)))
            if overlap > 0.55:
                return True
    if shafts_duplicate_pair(a, b):
        return True
    if allow_fragment_conflicts and (is_redundant_fragment(a, b) or is_redundant_fragment(b, a)):
        return True
    return False


def shafts_duplicate_pair(a: FittedShaft, b: FittedShaft) -> bool:
    if len(a.contacts_mm) == 0 or len(b.contacts_mm) == 0:
        return False
    d = np.linalg.norm(a.contacts_mm[:, None, :] - b.contacts_mm[None, :, :], axis=2)
    frac_a = float(np.mean(np.min(d, axis=1) < 1.6))
    frac_b = float(np.mean(np.min(d, axis=0) < 1.6))
    return frac_a > 0.60 or frac_b > 0.60
...
def extend_grid_by_evidence(
    ...
    for _ in range(max_extra_total):
        ...
        left_ok = left_ev >= base_ev_thr and (left_in or left_dist <= support_dist or left_ev >= strong_ev_thr)
        left_bad = (not left_in) and left_dist > outside_dist and left_ev < strong_ev_thr
        ...
        right_ok = right_ev >= base_ev_thr and (right_in or right_dist <= support_dist or right_ev >= strong_ev_thr)
        right_bad = (not right_in) and right_dist > outside_dist and right_ev < strong_ev_thr
...
def rebuild_shaft_from_contacts(
    ...
    brain_support = float(np.mean(inside | (dist <= support_dist)))
    inside_fraction = float(np.mean(inside))
    outside_fraction = float(np.mean((~inside) & (dist > outside_dist)))
...
def combine_shafts(a: FittedShaft, b: FittedShaft) -> FittedShaft:
    ...
    inside_fraction = weighted_average(
        [a.inside_fraction, b.inside_fraction],
        [len(a.contacts_mm), len(b.contacts_mm)],
    )
...
def evaluate_case(case: CaseRecord) -> dict:
    ...
    candidates = extract_candidates(ct_raw, ct_affine, surface)
    peak_chains = propose_chains(candidates)
    component_chains: list[ChainHypothesis] = []
    chains = peak_chains
    chains.sort(key=lambda x: x.score, reverse=True)
    chainopt_shafts = select_and_fit_shafts(chains, candidates, ct_raw, ct_affine, surface)
    greedy_shafts = baseline_detector.select_and_fit_shafts(chains, candidates, ct_raw, ct_affine, surface)
    chainopt_quality = shaft_set_quality(chainopt_shafts, candidates.surface_support_fraction)
    greedy_quality = shaft_set_quality(greedy_shafts, candidates.surface_support_fraction)
    if chainopt_quality >= greedy_quality - 0.10:
        shafts = chainopt_shafts
        chosen_mode = "chainopt"
    else:
        shafts = greedy_shafts
        chosen_mode = "greedy_fallback"
    ...
    return {
        ...
        "chainopt_quality": float(chainopt_quality),
        "greedy_quality": float(greedy_quality),
